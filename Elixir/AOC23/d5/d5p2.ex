...
zippedSeeds = List.zip([seedsInit |> Enum.take_every(2), seedsInit |> Enum.drop(1) |> Enum.take_every(2)])
[zi: zippedSeeds] |> IO.inspect()
seedsInit =  zippedSeeds |> Enum.map(fn pair -> Enum.to_list(elem(pair,0)..elem(pair,1)) end) |> List.flatten() |> IO.inspect()
